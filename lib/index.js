...
app.get('/park/occupy', async (req, res) => {
	...
	try {
		...
		const data = await parking.OpenStreetMapFetchRoadsAt(lat, lon, 12);
		...
		if (near) {
			...
			console.log(`User ${user_id} parked at [${lat}, ${lon}] was moved to [${near[2]}, ${near[3]}]`);
		} else {
			console.log(`User ${user_id} is parked illegally and his entry was nullified!`);
		}
